...
def Thedate():
    ...
    for root, dirs, files in os.walk(os.getcwd()+"/dan"):
        for a in files:
            if "csv" in a and re.findall("[0-9]",a):
                asda.append(a)
        #print(asda)
    asda.sort(key=lambda fn:os.path.getmtime(os.getcwd()+"/dan/" + fn))#按时间排序

    sCsvFileName ="./dan/"+asda[len(asda)-1]
    print(sCsvFileName)
    NAME_LINE   = 0
    STATE_LINE = 0
    LAIYUAN_LINE = 0
    ...
    #############################
    #查找规则
    fenlei=['淘宝','家家悦超市','王者荣耀','滴滴','新北洋超市','外卖','出去吃','衣服','鞋子','联通','待分类的','淘宝','回家路费','基金']
    RuleList=[
                [r'花呗','拼多多'],   #花呗
                [r'家家悦'],    #超市
                ...
                [r'平安','北洋','转账'],
                [r'美团点评','美团订单','饿了'],
                [r'烧烤','零食','饭','肠','肉蟹','火锅','周三刀','重庆炸酱面','光明晶品源','美食','牛杂','鸡柳'],
    ...
    groups=[0,1,0,1,1,1,1,0,0,0,2,0,0,0]
    ...
    excel_write.save(os.getcwd()+"/dan/"+'TheDateInfo.xls')
   ...
   # print(TheDate2)
    for i in range(len(TheDate2)):
        if len(TheDate2[i]):
            f= open(os.getcwd()+'//Class//'+fenlei[i]+'.csv','w',encoding="GBK")
            ...
            try:
                if os.path.exists(os.getcwd()+'//Class//'+fenlei[i]+'.csv') == True:

                    os.remove(os.getcwd()+'//Class//'+fenlei[i]+'.csv')
            except:
                print('删除失败')
